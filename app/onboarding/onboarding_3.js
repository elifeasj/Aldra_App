...
const Onboarding3 = () => {
    ...
    const [currentStep, setCurrentStep] = useState(3); // Start på første trin

    const handleNext = () => {
        if (currentStep < 3) { // Antal onboarding-sider
            setCurrentStep(currentStep + 1);
        } else {
            router.push('/onboarding/onboarding_4'); // Gå videre til næste onboarding
        }
    };
    ...
    const handlePrev = () => {
        if (currentStep > 3) {
            ...
            router.push('/onboarding/onboarding_2'); // Gå tilbage
        }
    };
    ...
    return (
        <View style={styles.container}>
            {/* Billede */}
            <Image
                source={require('../../assets/images/intro_3.png')} // Skift til dit billede
                style={styles.image}
            />

            <View style={styles.topContainer}>
                <Text style={styles.title}>Samarbejd med familien</Text>
                <Text style={styles.subtitle}>Invitér din familie dit Aldra-link</Text>
                <Text style={styles.description}>
                    Invitér familiemedlemmer til at deltage i deling af minder og få overblik over hvem 
                    der har delt med jeres kære.
                </Text>
            </View>
...
const styles = StyleSheet.create({
    ...
    image: {
        width: '80%',  // Billedets bredde fylder hele skærmen
